...
data and miscellaneous "rest" data. This version does not incorporate the
single-cell tracing data. """
...
def lnlike_BLI(lambS=lambS):
    ...
    residual = (data_rate - model_rate)/data_error
    lnlike = 0.5*residual**2
    return -lnlike
...
def lnprior(lambS=lambS, lambA=lambA, lamb1=lamb1, d1=d1, mu1=mu1):
    ...
    if (C1 and C2 and C3 and C4 and C5 and C6):
        return np.sum(np.log([pr1, pr2, pr4, pr5, pr6]))
    return -np.inf
...
def lnpost(theta):
    ...
    lambS, lambA, lamb1, d1, mu1 = theta
    ...
    lnprob = prior + like_BLI + like_rest + like_Ki67
    return lnprob
...
def MCMC_sample(walkers=100, steps=50000, lambS=lambS, lambA=lambA,
                lamb1=lamb1, d1=d1, mu1=mu1, diagnostics=False,
                storechain=True):
    ...
    fig = corner.corner(samples, labels=[r'$\lambda_S$ (1/day)',
                                         ...
                                         r'$d_1$ (1/day)',
                                         r'$\mu_1$ (1/day)'],
                        truths=[[lambS_MAP], [lambA_MAP], [lamb1_MAP], [d1_MAP], [mu1_MAP]],
    ...
    axes[0].plot(sampled_params[:, 0], -(lnprobs-best_post), 'o', color='darkgrey', alpha=0.1, rasterized=True)
    axes[0].set_title(r'$\lambda_S = {}_{{\ {}}}^{{\ {}}}$ 1/day'.format(r2(lambS_MAP),
        r2(lambS_5min), r2(lambS_5max)))
    axes[0].axhline(y=3, xmin=0., xmax=1, color='firebrick')
    axes[0].set_ylim([-0.2, 5])
    axes[0].set_ylabel(r'log $p(\theta | \theta_{{MAP}})$')

    axes[1].plot(sampled_params[:, 1], -(lnprobs-best_post), 'o', color='darkgrey', alpha=0.1, rasterized=True)
    axes[1].set_title(r'$\lambda_A = {}_{{\ {}}}^{{\ {}}}$ 1/day'.format(r2(lambA_MAP),
        r2(lambA_5min), r2(lambA_5max)))
    axes[1].set_xlim([0, 2])
    axes[1].axhline(y=3, xmin=0., xmax=1, color='firebrick')

    axes[2].plot(sampled_params[:, 2], -(lnprobs-best_post), 'o', color='darkgrey', alpha=0.1, rasterized=True)
    axes[2].set_title(r'$\lambda_1 = {}_{{\ {}}}^{{\ {}}}$ 1/day'.format(r2(lamb1_MAP),
    ...
    axes[3].plot(sampled_params[:, 3], -(lnprobs-best_post), 'o', color='darkgrey', alpha=0.1, rasterized=True)
    axes[3].set_title(r'$d_1 = {}_{{\ {}}}^{{\ {}}}$ 1/day'.format(r2(d1_MAP),
        r2(d1_5min), r2(d1_5max)))
    axes[3].axhline(y=3, xmin=0., xmax=1, color='firebrick')
    axes[3].set_xlim([-0.05, 2.2])

    axes[4].plot(sampled_params[:, 4], -(lnprobs-best_post), 'o', color='darkgrey', alpha=0.1, rasterized=True)
    axes[4].set_title(r'$\mu_1 = {}_{{\ {}}}^{{\ {}}}$ 1/day'.format(r2(mu1_MAP),
    ...
    if storechain:  # writes a the whole chain (if chain is short) or a
        ...
        df.to_hdf('figures/chain_{}_{}.h5'.format(walkers, steps), key='MCMC',
                  mode='w')
        dfMLE.to_hdf('figures/chain_{}_{}.h5'.format(walkers, steps), key='MLE',
                     mode='r+')
        dfUPPER.to_hdf('figures/chain_{}_{}.h5'.format(walkers, steps), key='UPPER',
                     mode='r+')
        dfLOWER.to_hdf('figures/chain_{}_{}.h5'.format(walkers, steps), key='LOWER',
                     mode='r+')
...
def find_CR(samples1D, lnprobs, alpha=0.05):
    ...
    best = np.max(lnprobs)
    # get allowed log difference from required credibility level
