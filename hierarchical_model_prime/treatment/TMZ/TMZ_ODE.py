...
def treatment_ODE(y, t, lambS, lambA, lamb1, d1, mu1, xi_TMZ, mu_TMZ):
    """ Growth dynamics of a complete tumour under TMZ administration.
    ...
    Args:
        y (array): Current values of each function.
        t (float): Current time point.
        [lambS, lambA, lamb1, d1, mu1, xi] (floats): Parameter values for
            the system.
    ...
    dy = np.zeros(4)
    dy[0] = 0  # qSC
    dy[1] = - xi_TMZ*(lambS+lambA)*y[1] - mu_TMZ*y[1]  # aSC
    dy[2] = - xi_TMZ*lamb1*y[2] - d1*y[2] - mu_TMZ*y[2]  # progeny
    dy[3] = d1*y[2] - mu1*y[3] - mu_TMZ*y[3]  # exhausted
    return dy
